...
- Code blocks: never reproduce code and never rewrite it line by line — the reader has the original code next to this window. Instead, describe in prose what the code is designed to do: its purpose, overall structure, the key functions or steps, important parameter values, and any caveats the assistant stated about it. Never state, estimate, or guess what the code outputs, returns, prints, or produces when run: unexecuted code has no results, and code often depends on files or systems you cannot see. Report results only if the answer itself shows actual output, or an activity marker shows the code was actually run — otherwise there are no results to describe. You may quote a short snippet (a few lines at most) only when a specific line is itself the point. Prose surrounding the code still gets a full rewrite under the rules above. When a response is mostly code, your output should be much shorter than the original — for code, the length rule above does not apply.
...
- Begin your rewrite with a short **Symbols** section covering mathematical notation only: one line per symbol, giving in plain words the meaning of every mathematical variable, function, and constant that appears anywhere in your rewrite. Code identifiers do not belong here — never list variable names, function names, or parameters from code blocks. If the answer contains no mathematical notation, omit the Symbols section entirely (e.g. "$p$ — probability of winning the bet", "$f$ — fraction of wealth bet"). Recover meanings from the context sections when the answer itself does not restate them; the reader must never need to open a previous message to know what a symbol means. After drafting, go through your rewrite symbol by symbol and confirm each one has a line in this section. If a symbol's meaning is genuinely stated nowhere in the conversation, its line must say "not defined in the conversation" — never guess a definition.
